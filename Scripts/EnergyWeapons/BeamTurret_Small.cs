...
{
	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_InteriorTurret), true, "CythonSmallBlockSmallLaserTurret")] 
	public class BeamTurret_Small: MyGameLogicComponent
	{
		...
		private void getBeamWeaponInfo(string name) {
			...
			if (subtypeName == "CythonSmallBlockSmallLaserTurret") {
				beamWeaponInfo = EnergyWeaponManager.smallBlockSmallLaserTurret;
			}
		}
		...
		public override void UpdateBeforeSimulation ()
		{
			...
			{
				var beamcolor = Color.MediumSeaGreen;
				var beamcolor_aux = Color.LightCyan;
				...
				if(hitBool == false)
				{
					if(!MyAPIGateway.Utilities.IsDedicated)
					{
						if(!MyAPIGateway.Session.CreativeMode)
						{
							VRage.Game.MySimpleObjectDraw.DrawLine(from, to, material, ref auxcolor, 0.15f * (currentHeat/beamWeaponInfo.maxHeat + 0.2f));
							VRage.Game.MySimpleObjectDraw.DrawLine(from, to, material, ref maincolor, 0.5f * (currentHeat/beamWeaponInfo.maxHeat + 0.2f));
						}
						else
						{
				...
				else
				{
					if(!MyAPIGateway.Utilities.IsDedicated)
					{
						if(!MyAPIGateway.Session.CreativeMode)
						{
							VRage.Game.MySimpleObjectDraw.DrawLine(from, toTarget, material, ref auxcolor, 0.15f * (currentHeat/beamWeaponInfo.maxHeat + 0.2f));
							VRage.Game.MySimpleObjectDraw.DrawLine(from, toTarget, material, ref maincolor, 0.5f * (currentHeat/beamWeaponInfo.maxHeat + 0.2f));
						}
						else
						{
							VRage.Game.MySimpleObjectDraw.DrawLine(from, toTarget, material, ref auxcolor, 0.15f * 1.2f);
							VRage.Game.MySimpleObjectDraw.DrawLine(from, toTarget, material, ref maincolor, 0.5f * 1.2f);
						}
					}
				}
			}
